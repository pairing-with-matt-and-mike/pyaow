...
test("matrix equality with identical matrices", () => {
  const rows = [[1, 2, 3, 4], [2, 3, 4, 5], [3, 4, 5, 6], [4, 5, 6, 7]];
...
test("multiplying two matrices", () => {
  const a = matrix([[1, 2, 3, 4], [2, 3, 4, 5], [3, 4, 5, 6], [4, 5, 6, 7]]);
  const b = matrix([[0, 1, 2, 4], [1, 2, 4, 8], [2, 4, 8, 16], [8, 8, 16, 32]]);
  const c = matrix([
    [40, 49, 98, 196],
    [51, 64, 128, 256],
    [62, 79, 158, 316],
...
test("a matrix multiplied by a tuple", () => {
  const A = matrix([[1, 2, 3, 4], [2, 4, 4, 2], [8, 6, 4, 1], [0, 0, 0, 1]]);
...
test("transposing a matrix", () => {
  const A = matrix([[0, 9, 3, 0], [9, 8, 0, 8], [1, 8, 5, 3], [0, 0, 5, 8]]);

  const At = matrix([[0, 9, 1, 0], [9, 8, 8, 0], [3, 0, 5, 5], [0, 8, 3, 8]]);
...
test("a submatrix of a 4x4 matrix is a 3x3 matrix", () => {
  ...
  const E = matrix([[-6, 1, 6], [-8, 8, 6], [-7, -1, 1]]);
...
test("testing an invertible matrix for invertibility", () => {
  const A = matrix([[6, 4, 4, 4], [5, 5, 7, 6], [4, -9, 3, -7], [9, 1, 7, -6]]);
...
test("multiplying a matrix by its inverse", () => {
  ...
  const B = matrix([[8, 2, 2, 2], [3, -1, 7, 0], [7, 0, 5, 4], [6, -2, 0, 5]]);
  expect(
    A.mul(B)
      .mul(B.inverse())
      .equals(A, 0.00001)
  ...
  expect(
    A.transpose()
      .inverse()
      .equals(A.inverse().transpose(), 0.00001)
  ).toBe(true);
});
