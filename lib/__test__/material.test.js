...
const { stripePattern } = require("../patterns");
...
test("lighting with the eye between the light and the surface", () => {
  ...
  const s = sphere();
  const result = m.lighting(light, s, position, eyev, normalv);
...
test("Lighting with a pattern applied", () => {
  ...
  const s = sphere();
  const c1 = m.lighting(light, s, point(0.9, 0, 0), eyev, normalv, false);
  const c2 = m.lighting(light, s, point(1.1, 0, 0), eyev, normalv, false);
