...
class Material {
  ...
  lighting(light, object, point, eyev, normalv, options = {}) {
    const { inShadow = false } = options;
    const color = this.pattern
      ? this.pattern.colorAtObject(object, point)
      : this.color;
    const effectiveColor = hadamardProduct(color, light.intensity);
